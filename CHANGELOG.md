...
### 2026-07-18
- `fe8c855` feat(comlink): навигация по разделу карточками HubCard на индексе
- `c8cdb3f` docs: сверка прогресса + перенос выполненного в done/
- `f9d6d17` refactor(comments): полиморфный слой обсуждений + 4 новые поверхности
- `e1f8329` feat(loadouts): комментарии к сборкам с рейтингом «Полезно»
...
- `c338603` fix(profile): убран дубль «· ЦТА» в title и род месяца в дате
...
- `e12d66c` feat(comlink): верификация ЧВК-профиля — галочка через модерацию
- `f437a1e` design(players): нативная профиль-панель из готовых блоков ЦТА
- `28f24f2` feat(players): полный синк ЧВК — +уровень (по XP-таблице) и издание
- `6dfd454` feat(players): кнопка «Обновить профиль ЧВК» из загруженного JSON
...
- `4d25738` fix(account): бренд-иконки в блоке «Публичные хендлы» вместо текстовых инициалов
- `29e873a` perf(egress): карты-подложки готовы к переезду на R2 (снять оставшийся сток Supabase)
- `c4c0092` feat(auth): рабочий вход через Discord/Twitch + управление в Аккаунт Центре
- `a1855e1` ci(vercel-ignore): пропускать билд и для CHANGELOG.md-only коммитов
- `d10e64c` polish(seasons): анимации и эффекты Генератора вызова
- `6d516a2` feat(seasons): «Генератор вызова» — интерактивный каталог билдов перков
- `b68fe75` feat(barter): «Поток сделок» — играбельная колода бартеров вместо строки поиска
- `69e66cd` feat(comlink): временные иконки-плейсхолдеры для подразделов «Связи»
- `5648539` feat(comlink): верхняя навигация-переключатель во всех подразделах «Связи»

### 2026-07-16
- `056802c` refactor(game-updates): убрать англ. выжимку BSG из ленты патчей
- `4666a2b` refactor(nav): «Обновления игры» переехали из «Связи» в «Кодекс»
- `7ad09c0` feat(game-changes): плашки «Недавно изменено» на странице предмета
