...
const nextConfig: NextConfig = {
  ...
  // Открыть dev-сервер телефону в своей Wi-Fi сети (проверка 3D-карты на живом
  // устройстве: эмулятор врёт про производительность). Next с 15.3 режет запросы
  // к dev-ассетам с чужого origin — без этого списка телефон получает страницу
  // без стилей и скриптов. Только для `next dev`, на прод не влияет.
  // ⚠️ ЭТОТ СПИСОК ЗАМЕЩАЕТ УМОЛЧАНИЕ, А НЕ ДОПОЛНЯЕТ ЕГО. Стоило вписать сюда
  // один лишь адрес телефона — и dev-ресурсы (/_next/hmr, клиентские чанки)
  // оказались закрыты для localhost: страницы отдавали 200, а динамические
  // компоненты навсегда зависали в состоянии загрузки. Симптом обманчивый —
  // выглядит как «не собирается three.js», хотя так же висел и Leaflet.
  // Поэтому localhost и 127.0.0.1 держим в списке явно.
  allowedDevOrigins: ['localhost', '127.0.0.1', '192.168.1.2', '192.168.1.*'],
