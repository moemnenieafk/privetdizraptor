 ...
 *  ВАЖНО: без литералов вида `name-[...]` / `var(...)` — Tailwind v4 сканит контент и
 *  генерит из них (порой невалидный) CSS. Описываем долг словами, не классами. */
function renderBlock(m) {
  ...
  return (
    ...
    `сырой HEX в произвольных значениях — ${m.hexN} шт / ${m.hexFiles} файлов · ` +
    `произвольный rounded в px — ${m.roundedN} шт · ` +
    `v3-утечки (gradient-направление, var() в bg) — ${v3}.`
  );
}
