...
# ⬜ РЕНТГЕН. Стены становятся полупрозрачными, чтобы показать начинку здания —
# марши лестниц физически есть, но их закрывает собственная застройка.
# Пол при этом ОСТАЁТСЯ ПЛОТНЫМ: сплошное стекло превращает здание в аквариум,
# где не читаются уровни. Плита чуть темнее стен — она читается как основание.
M_XRAY = mat('xray-wall', (0.190, 0.196, 0.212, 1.0), alpha=0.20)
M_PLATE = mat('floor-plate', (0.105, 0.108, 0.118, 1.0))
...
for g in JOB['groups']:
    ...
    # 🔴 ГОЧА, найденная по габаритам объектов в логе, а не глазами.
    # Solidify с offset=1.0 гонит объём ПО НОРМАЛИ, а recalc_face_normals на
    # НЕСВЯЗНЫХ плоских гранях (у нас это сотни отдельных контуров этажа)
    # ориентирует их вразнобой: «наружу» для одинокой плоскости не определено.
    # Итог — половина контуров выдавливалась ВНИЗ, и объект этажа занимал
    # z0 ± h вместо z0 … z0+h. Замер по Таможне: main давал z[-2.8..2.8],
    # 2nd z[0.5..5.9] — соседние этажи перекрывались на 2.3 м из 3.2, здание
    # читалось слипшейся кашей плашек вместо стопки этажей. Дефект был во ВСЕХ
    # превью до 06.09 и не бросался в глаза, потому что сверху перекрытие не
    # видно — его выдал только габарит по Z в логе.
    # Пол этажа горизонтален по построению, поэтому направление задаётся жёстко:
    # все нормали вверх.
    # ШАГ 1 — выбросить вырожденные грани. У треугольника нулевой площади нормаль
    # тоже нулевая, развернуть его нельзя: направления у него нет. Solidify такие
    # грани всё равно обрабатывает и утаскивает ВНИЗ. Их тем больше, чем ниже
    # порог шума, поэтому снижение порога ради тонких контуров (железные лестницы)
    # ломало этажи один за другим — гарда высоты поймала это на первом прогоне.
    # Критерий по площади мало: остаётся горстка почти-вертикальных слив, у
    # которых площадь чуть выше нуля, а нормаль лежит в плоскости. Одной такой
    # грани хватало, чтобы утащить габарит ВСЕГО этажа вниз на толщину — так
    # 2-й этаж ломался на всех порогах подряд. Поэтому критерия два.
    junk = [f for f in bm.faces
            if f.calc_area() < 1e-6 or abs(f.normal.z) < 1e-6]
    if junk:
        bmesh.ops.delete(bm, geom=junk, context='FACES')
        log('  вырожденных граней удалено: %d' % len(junk))
    # Удаление граней оставляет ОСИРОТЕВШИЕ вершины и рёбра. У них нет нормали,
    # а Solidify всё равно смещает их — и они раздувают габарит объекта, хотя
    # в кадре их не видно. Чем ниже порог шума, тем больше сирот, поэтому
    # наземный этаж ломался только на низких порогах.
    loose = [v for v in bm.verts if not v.link_faces]
    if loose:
        bmesh.ops.delete(bm, geom=loose, context='VERTS')
        log('  осиротевших вершин удалено: %d' % len(loose))
    # ШАГ 2 — развернуть остальные вверх. Пол этажа горизонтален по построению,
    # поэтому направление задаётся жёстко, а не выводится эвристикой.
    flipped = 0
    for f in bm.faces:
        if f.normal.z < 0.0:
            f.normal_flip()
            flipped += 1
    if flipped:
        log('  нормалей развёрнуто вверх: %d из %d' % (flipped, len(bm.faces)))
    # Приёмка собственной работы: после двух шагов ВСЕ грани обязаны смотреть вверх.
    bad = sum(1 for f in bm.faces if f.normal.z <= 1e-6)
    if bad:
        log('  ⚠ граней без верхней нормали после разворота: %d' % bad)
    bm.to_mesh(me)
    bm.free()
    me.update()
    ...
    elif JOB.get('xray') and g['tag'] == 'build':
        ob.data.materials.append(M_XRAY)
    else:
        ob.data.materials.append(M_BUILD if g['tag'] == 'build' else M_PROP)
    scene.collection.objects.link(ob)
    # Ожидаемый габарит по Z кладём НА ОБЪЕКТ: проверить его можно только после
    # того, как Solidify отработает, то есть в блоке диагностики перед рендером.
    ob['z_expect'] = (g.get('z0', 0.0), g.get('z0', 0.0) + g['height'])
...
for o in scene.collection.objects:
    if o.type == 'MESH':
        ...
        # 🛡 ГАРДА ВЫСОТЫ. Дефект «половина контуров этажа выдавливается ВНИЗ»
        # жил во ВСЕХ превью до 06.09 и глазами не ловился: сверху перекрытие
        # этажей не видно, здание просто выглядит «слипшимся». Выдал его только
        # габарит по Z в логе — значит и проверять надо числом, а не кадром.
        # bound_box считается ПОСЛЕ Solidify, поэтому сверка стоит здесь, а не
        # в месте сборки объекта, где меш ещё плоский лист.
        exp = o.get('z_expect')
        if exp is not None:
            z0e, z1e = float(exp[0]), float(exp[1])
            if min(zs) < z0e - 0.05 or max(zs) > z1e + 0.05:
                log('   ⚠ ГАРДА ВЫСОТЫ %s: факт z[%.2f..%.2f], ждали z[%.2f..%.2f]'
                    % (o.name, min(zs), max(zs), z0e, z1e))
