...
- **Bottle of Yxmc Water `6a3557f8` ✅ ЗАЛИТА** (R2 HTTP 200, силуэт верный). Причина была НЕ «фикс пивота еды» (диагноз ПРОДОЛЖЕНИЯ-3 неверен), а **неверный бандл**: превью-пивот + модель у еды/напитков лежат в **`_loot`**-бандле, а карта указывала на `_container` (hands-префаб, PreviewPivot там НЕТ вовсе). Фикс: `6a3557f8`→`item_water_bottle_loot.bundle` в `src/data/icon-bundle-map-eft.json` + скип физ-прокси (`collider`/`collision` по имени GO) в `extract_item.py` (у лут-моделей еды видимый GO «collider» лез коробкой в кадр). **Рецепт для др. еды/напитков: маппить `*_loot.bundle`, НЕ `*_container`.**
