...
> ## 🔴 ТЕКУЩЕЕ СОСТОЯНИЕ (2026-08-05) — ОСТАЛОСЬ **8**
> **Оружие ЗАКРЫТО (7/7). Вода + Admin's key ЗАКРЫТЫ.** Осталось **8 брони-скинов** (Stripped) — ждут подтверждения маппинга V4DYA по контакт-листу арена/вариант-бандлов (`scripts/reports/_armor_candidates.png`). Admin's key `6a33c179` = `item_key_9` (V4DYA опознал по контакт-листу `_key_candidates.png`). Метод остатка: probe-рендер вариант-бандлов → контакт-лист → глаз V4DYA → залив подтверждённого.
...
> Свежий дифф-критерий: `node scripts/icon-render/diff_r2_missing.mjs` (было 60 → **8**).
