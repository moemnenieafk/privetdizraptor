...
> ## 🔴 ТЕКУЩЕЕ СОСТОЯНИЕ (2026-08-05) — ОСТАЛОСЬ **3**
> **Оружие (7/7) + вода + Admin's key + 5 брони-камо ЗАКРЫТЫ.** Осталось **3 гладиатор-СПЕЦ-скина**: `31` Death is Inevitable · `42` Viking · `6e` Gray (Fort Gladiator S Stripped). Премиум-арена-скины — offline не опознать без глаза V4DYA/референса (урок воды). Ждут его маппинга по `scripts/reports/_glad.png` (5 вариантов) ЛИБО выноса в offline-список.
> Залито 5 брони через вариант-бандлы (arena/winter): `33` AVS-Multicam→`cryeavs_arena_bp_01`, `3b` M2-Alpine→`m2_winter`, `71` Kirasa-Green→`kirasa_arena_bp_01`, `32` TacTec-Storm→`carriertactec_arena_bp_01`, `49` Trooper-Coyote→`trooper_arena_bp_01`. Admin's key = `item_key_9`.
...
> Свежий дифф-критерий: `node scripts/icon-render/diff_r2_missing.mjs` (было 60 → **3**; остаток = только премиум-скины без offline-модели).

## ⚠ БИТЫЕ РЕНДЕРЫ — дифф их НЕ ловит (2026-08-05, урок)
`diff_r2_missing.mjs` = existence-only (каталог − R2). **Битый рендер (чёрный блоб без текстур) диффом НЕ виден** — иконка ЕСТЬ, но негодная. V4DYA нашёл глазом: **AK-308 RMR `707265…021`** был чёрным блобом (в карте — голый `weapon_ak308_container` БЕЗ сборки → рендер без обвеса/оптики). Фикс: собрал по дереву 27 (Vudu) МИНУС Vudu-оптика (`eotech_vudu` + маунт `super_precision`) → полный АК-308 с RMR-редотом (RMR уже был в дереве 27!), перезалит в R2 (HTTP 200). Дерево 21 → `full-weapon-trees.json` (харнесс). **TODO глаз-аудит остальных weapon-рендеров на чёрные блобы** (особенно где в карте голый `*_container` без assembly-map).

## ✅ ПРОДОЛЖЕНИЕ-5 (2026-08-05, этот чат) — вода исправлена + key закрыт + 5 брони залито → ОСТАЛОСЬ **3** (гладиатор-скины)
Автономный остаток исчерпан → **ревью-пасс**: probe-рендер вариант-бандлов → контакт-лист → глаз V4DYA → залив подтверждённого.

**✅ Закрыто (R2 HTTP 200):**
- **Вода Ymxc `6a3557f8`** — исправлена (был не тот предмет, см. ПРОДОЛЖЕНИЕ-4: скелет-Aquamari, 3 фикса).
- **Admin's key `6a33c179` → `item_key_9`** (V4DYA опознал по `_key_candidates.png`). Карта в `src/data/icon-bundle-map-eft.json`.

**⚠ КОРРЕКЦИЯ вывода П-4 по броне:** «оффлайн не рендерятся» — неверно для КАМО-вариантов. Спец-скины по ИМЕНИ (death/viking) в каталоге нет, НО камо-версии носителя лежат ОТДЕЛЬНЫМИ бандлами с суффиксом (`_arena_bp_01`, `_winter`) и рендерятся штатно. Реальный расклад 8 брони-Stripped:
- **Резолвятся вариант-бандлом (ждут глаза V4DYA), моя привязка:** `33` AVS Multicam→`rig_cryeavs_arena_bp_01`; `3b` ANA M2 Alpine→`rig_m2_winter`(снежный); `71` Kirasa Green→`armor_kirasa_arena_bp_01`(зелёный, база синяя); `32` TacTec Storm→`rig_carriertactec_arena_bp_01`(тёмный); `49` Trooper Coyote→`armor_trooper` base/arena (оба мультикам-ish — спорно).
- **Спец-скины гладиатора (вероятно вынос):** `31` Death / `42` Viking / `6e` Gray — среди 5 гладиатор-вариантов (base/champion/mltcm/light_blk/light_multicam) явного death/viking/gray НЕТ → рантайм-оверрайд.

**Артефакты ревью (`scripts/reports/`, gitignore):** `_probe_map.json` (probe-id→бандл, 15 армор + 16 ключ-кандидатов + probe_water_skeleton), `_armor_candidates.png` (15 брони с метками), `_key_candidates.png` (16 ключей). Probe-рендеры в `out-unity/probe_*`.

**СЛЕДУЮЩИЙ ШАГ:** V4DYA подтверждает маппинг брони строкой (формат как ключ: «33=avs arena, 3b=m2 winter, …») → под каждый подтверждённый: probe-бандл в `icon-bundle-map-eft.json` под РЕАЛЬНЫМ preset-id (`707265…`) → рендер real-id → `upload_r2_ids.mjs` → verify HTTP 200. Неподтверждённые (гладиаторы) → отдельный список «offline-нерендерятся» (критерий готово это допускает).

**Фиксы пайплайна этой сессии (git, ветка road-to-release):** `extract_item.py` (скип `collider` + material-скоринг скин-принта), `render_unity.mjs` (`usable_items`=plain). Коммиты `d8512d45`, `e29cf6e1`, `6acdba01`. Скилл прокачан (food/drink рецепт, урок name-матча).

## ✅ ПРОДОЛЖЕНИЕ-4 (2026-08-05, этот чат) — вода ЗАКРЫТА
...
- **8 брони-скинов + Admin's key → УТОЧНЕНО/ЗАКРЫТО в ПРОДОЛЖЕНИИ-5 (выше).** Тогдашний вывод «броня оффлайн не рендерится» верен ТОЛЬКО для спец-скинов гладиатора (death/viking/gray); камо-варианты (arena/winter-бандлы) рендерятся штатно. Admin's key закрыт (`item_key_9`).
