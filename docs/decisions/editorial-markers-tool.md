...
- **Маркировка (2026-07-30)** — избавились от «?»: пикер категорий из общей таксономии сайта
  (`categories.ts`): типы (выход/спавн/лут/контейнер/переход/опасн./замок/рычаг/стационарка/зона
  задания/POI) + подкатегории (выход→фракция [+колонка `faction`], спавн→12 фракций, лут/контейнер→
  группы с иконками, опасности→снайпер/мина/миномёт/растяжки, зона→цель/предмет). Рендер через
  `manualMarkerIcon(type+category+faction+meta)` — те же реальные иконки, что у ручных/синканных.
- **Сюжет↔маркеры** — привязка к истории (Без/Квест/Сюжет+шаг), чип СЮЖЕТ летит к пинам (📍N),
  индикатор истории в карточке, цвет пина по типу привязки (poi-фолбэк).

Осталось опционально: floor-фильтр editorial-слоя на мультиэтажках, чистка демо-маркера.
