---
status: ✅ ИСПОЛНЕНО — HD-Завод боевой на проде (2026-08-13, autopilot); Ш4/6 уборка опц., оставлена
...
## Критерий готовности
- [x] Тайлы на R2, карта грузит их (не `/public`) — локально + **прод** (network 16/16 тайлов `?v=6` = 200 с r2.dev).
- [x] `/eft/maps/factory` = HD (тайлы+editorial), рендер + drawer'ы + этажи + визард (браузер: прод + локал).
- [x] Метки — модель **A** (полностью editorial): 446 мигрированы; синканные оставлены дормантом (тайл-ветка их не рендерит).
- [x] Редирект `factory-hd→factory` (прод: 308). ⚠️ *Удаление старой Shebuka `Factory.svg` — опц. уборка (Ш6), НЕ сделана: орфан безвреден, оставлен.*
- [x] Мерж в main (`5b249c48`, fast-forward) + деплой Vercel (success) + **ЖИВОЙ сайт верифицирован**.
...
**Шаг 7 ✅ (2026-08-13, autopilot — V4DYA ушёл спать, сняв §4.3/§5) — на проде.** Коммит `5b249c48`
(16 файлов, 2 чужих untracked-файла исключены), push feat → **fast-forward main** (`f6cd8585..5b249c48`) →
Vercel деплой **success**. **Vercel env НЕ понадобился:** `mapAssetBase` получил фолбэк-цепочку до
`NEXT_PUBLIC_ICON_BASE_URL` (тот же R2-бакет, в проде уже задан — иконки с него) → прод отдаёт тайлы с R2 без
ручной правки дашборда. Прод-вериф: `factory-hd`→308→`factory`; `/eft/maps/factory` SSR = tiled-config (tileBase/
staticMap/«Завод»/v6); браузер на живом сайте — HD-тайлы + 446 меток рендерятся; network 16/16 тайлов `?v=6` = 200.

**Осознанно НЕ сделано (опц. уборка, безопасно оставлено дормантом):** Ш4 (пропуск factory в кроне — синканные и так
не рендерятся тайл-веткой, крон промоутнутую карту не трогает) и Ш6-удаления (Shebuka `Factory.svg` на R2/public;
синканные метки; строка maps `factory-hd`). Всё орфанное/дормантное, на UX не влияет; удалять под спящего V4DYA без
нужды не стал. Откат при необходимости: revert merge + redeploy (тайлы/метки на R2/БД остаются, безвредны).
