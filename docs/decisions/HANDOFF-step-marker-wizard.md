...
- **Ф5 остаток (синканный оверрайд) — СТОП: необратимое.** Требует alter `editorial_markers += source_marker_id`
  + `npx tsx scripts/migrate-all-ddl.ts` на прод (§5). Move уже есть; осталось: клик по СИНКАНОМУ tarkov.dev-
  маркеру → карточка/оверрайд (`source_marker_id`), мерж-патч на рендере, синк не трогает. Развилки из спеки:
  полный редакт vs аннотации; флаг «скрыть»; `source_marker_id` vs отдельная таблица (рекомендую source_marker_id).
