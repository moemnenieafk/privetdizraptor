...
- **Ф3 · Враг Шаг 2 (полный ростер боссов) — ждёт схему.** Figma 03-1 показывает 6 generic-иконок +
  ПОЛНЫЙ ростер боссов-портретов (reshala/gluhar/killa/…). Резолвер умеет `bossKey`→портрет, но в
  `editorial_markers` колонки `bossKey` НЕТ → нужен либо alter (§5), либо хранить boss в `category`
  (`category='reshala'` + твик резолвера, без миграции). **Рекомендую category-хак** (быстрее, обратимо).
  Пока «Враг» на `SPAWN_CATEGORIES` (12 фракций, боссы generic). Одно слово — alter или category-хак.
