...
**Главный вывод: CTA — НЕ дырявый вайб-проект. 12 из 15 пунктов уже закрыты на уровне архитектуры** (defense-in-depth: RLS + app-level auth + rate-limit + CAPTCHA + magic-bytes + серверный пейвол). Это результат зрелого бэкенда (см. `docs/decisions/done/hosting-autonomy-migration.md`, `cta-backend`-скилл), а не случайность.
