...
status: parsed-clean
repo_path: src/lib/vision/grid.ts
lines: 224
created: 2026-08-24
---
...
Геометрия сетки через автокорреляцию профиля рёбер, сегментация занятых ячеек в прямоугольники, кроп слота. Здесь же `GRID_TUNING` — все пороги сегментации.
...
Сырой файл: `_src/src/lib/vision/grid.ts`. Копируется в репозиторий по пути `src/lib/vision/grid.ts`.
