...
status: parsed-clean
repo_path: src/app/api/vision/inventory/route.ts
lines: 125
created: 2026-08-24
---

# `src/app/api/vision/inventory/route.ts`

Оркестрация пайплайна. Отказ Gemini не роняет скан — pHash-результаты к этому моменту уже собраны и отдаются как есть.
...
export async function POST(request: Request): Promise<NextResponse<ScanResponse | ScanFailure>> {
  ...
  const verdict = consume(clientKey);
  if (!verdict.allowed) {
  ...
  await db
    ...
    .values({ imageHash, result, cellPitch: geometry.pitch, visionCalls })
    .onConflictDoNothing();
...
Сырой файл: `_src/src/app/api/vision/inventory/route.ts`. Копируется в репозиторий по пути `src/app/api/vision/inventory/route.ts`.
