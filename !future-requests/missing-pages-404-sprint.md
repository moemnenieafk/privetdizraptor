...
> - **Кодекс-лор добивка — ГОТОВО (build green; runtime 200).** Ещё 2 статьи через `codex-lore-research-2`: `locations.ts` «Локации» (8 секций, 11 карт) + `theories.ts` «Теории и загадки» (7 секций, формат «Канон/Подтверждено/Спекуляции» с явными пометками). Реестр `index.ts` пересобран на 6 статей.
> - **Кодекс-хвост (audiotapes + docs-notes) — ГОТОВО (tsc=0; runtime 200/200; несуществующий слаг 404).** 2 статьи `src/data/codex/{audiotapes,docs-notes}.ts` + регистрация в реестре (теперь 8 статей). Контент через research-агент (web-разведка + адверсариальный фактчек): «Аудиозаписи» (SMT≠сюжетные плёнки, культ «Синего пламени», Rus Post, учёные TerraGroup/Лабиринт/Борей, Broadcast/Slaughter, радиоэфир/Network Provider) и «Документы и записки» (дело TerraGroup для «Билета», мисс А./«Большая Семёрка», Объект 1156/ЭМИ, журналы/кейсы, дневники/разведпапки, записки фракций). Канон/спекуляции размечены явно, пасхалки секьюрных папок (Dharma/Aperture/Ω) уточнены, без внешних ссылок. **Кодекс закрыт полностью.**
...
> - **Трекинг «построено» — ГОТОВО (build green; tsc/runtime ок).** Стор `useHideoutStore` (уровни станций, persist `cta-hideout`); `getHideoutStations()` (канон-список + maxLevel); переиспользуемая панель `HideoutLevelsPanel` (сеттер «Моё убежище» ±). **needed-Убежище** пересчитывает «осталось» (источники с level > построено); **craft-profit** — тоггл «Доступные сейчас» + индикатор ✓/🔒 по построенному уровню. Усиливает обе свежие страницы.
>
...
> - ⬜ Осталось:
>   - Phase 4 карты → `map_assets`/`map_markers`+изображения (бэкенд-хостинг). ~~кодекс: audiotapes/docs-notes~~ → ✅ ГОТОВО (день 3). **Карты — единственный незакрытый кусок.**
>
