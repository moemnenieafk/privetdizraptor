...
export const GRID_TUNING = {
   ...
   * Откалибровано на реальном скрине 1440p (docs/CTA-Vision-Spec/screenshots/eft-stash.jpg,
   * scripts/vision-calibrate.ts): при 9 фон/градиент/текстовые подписи между рядами читались
   * как «занято» и склеивались в ложные вертикальные полосы (1×12, 1×13). Порог 22 убирает
   * все ложные крупные склейки (big≥4: 13→0), оставляя реальные иконки предметов.
   */
  emptyStdDev: 22,
  /**
   * Доля тёмных пикселей на внутренней границе, при которой считаем это разделителем.
   * Откалибровано на 1440p: 0.6 надёжнее режет соседние 1×1 в плотной сетке, чем 0.7,
   * не порождая ложных разрезов на реальных предметах.
   */
  separatorDarkRatio: 0.6,
  separatorLuma: 70,
