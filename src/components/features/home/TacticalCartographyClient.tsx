  ...
  return (
    <section className="w-full px-4 md:px-8 pb-16">
      <div className="max-w-275 mx-auto">
        ...
        {/* Split layout */}
        <div className="border border-lines-hover rounded-xs flex flex-col lg:flex-row overflow-hidden">
          {/* LEFT — task list */}
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-3 group transition-none ${
                      hoveredTask?.id === task.id
                        ...
                        : "border-l-2 border-l-transparent hover:bg-lines-hover/30"
                    }`}
                  ...
                  <div className="flex flex-col gap-1.5">
                    {tasksOnHoveredMap.map((t) => (
                      <div key={t.id} className="flex items-center gap-2">
                        <div
                          className={`w-1.5 h-1.5 shrink-0 rounded-xs ${
                            t.id === hoveredTask.id ? "bg-(--primary)" : "bg-lines-hover"
                          }`}
                        />
                        <p
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="px-3 py-3 border-l-2 border-l-transparent">
          <div className="flex items-start justify-between gap-2 mb-1.5">
            <div className="h-3 bg-lines-hover animate-pulse w-32 rounded-xs" />
            <div className="h-4 bg-lines-hover animate-pulse w-16 rounded-xs" />
          </div>
          <div className="h-2.5 bg-lines-hover animate-pulse w-20 rounded-xs" />
        </div>
      ))}
    </>
  );
