...
// Кнопка «Сброс достижений» + модалка подтверждения (по аналогии со сбросом прогресса
// ЧВК: ProfileSettingsModal → ProfileResetModal). Чистит ТОЛЬКО домен достижений:
// useAchievementStore.resetProgress() (completed + tracked). Облако для залогиненных
// обновит AchievementSync автоматически (подписка на стор → дебаунс-PUT пустого стейта).
// Используется на странице достижений (ряд счётчика) и во вкладке «Трекинг».
import { useState } from 'react';
import { useModalAnimation } from '@/hooks/useModalAnimation';
import { useAchievementStore } from '@/store/useAchievementStore';

function AchievementResetModal({
  isOpen,
  onClose,
  onConfirm,
}: {
    ...
    >
      ...
      >
        ...
        <div className="relative flex h-7 w-full items-center justify-start gap-1 rounded-t bg-danger">
          ...
          <div className="text-sm font-blender-medium leading-4 text-zinc-100">Подтверждение сброса достижений</div>
  ...
  return (
    <>
      <button
        ...
        title="Сбросить прогресс достижений"
      ...
      >
        ...
        <span className="hidden text-xs font-blender-medium leading-3 text-danger sm:inline">СБРОС ДОСТИЖЕНИЙ</span>
      </button>

      <AchievementResetModal isOpen={open} onClose={() => setOpen(false)} onConfirm={resetProgress} />
    </>
  );
}
