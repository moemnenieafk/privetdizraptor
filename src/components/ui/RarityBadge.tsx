...
export function RarityBadge({ rarity, label, className = "" }: RarityBadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded border px-2 py-0.5 font-blender-medium text-type-caption uppercase tracking-widest ${config.classes} ${className}`}
    >
      {label || config.label}
    </span>
  );
