...
export function Tooltip({ children, content, position = "top", className = "" }: TooltipProps) {
  ...
  return (
    <div className={`group/tooltip relative inline-flex ${className}`}>
      ...
      >
        <div className="scale-95 rounded border border-lines-hover bg-card-menu px-2.5 py-1 text-type-caption font-blender-medium uppercase tracking-widest text-text-primary shadow-lg transition-transform duration-300 ease-out group-hover/tooltip:scale-100">
          {content}
        </div>
      </div>
    </div>
