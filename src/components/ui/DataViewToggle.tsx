...
export function DataViewToggle({ view, onChange, className = "" }: DataViewToggleProps) {
  ...
  const activeClasses = "bg-[color-mix(in_srgb,var(--primary)_20%,transparent)] text-[var(--primary)] shadow-[0_0_10px_color-mix(in_srgb,var(--primary)_40%,transparent)]";
  const inactiveClasses = "text-[var(--color-text-muted)] hover:bg-[color-mix(in_srgb,var(--color-card-menu)_50%,transparent)] hover:text-[var(--color-text-primary)]";

  return (
    <div className={`inline-flex items-center gap-1 rounded border border-[var(--color-lines-hover)] bg-[var(--color-base)] p-1 ${className}`}>
      <button
