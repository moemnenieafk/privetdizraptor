// Кнопка «Поддержка» (Boosty-донат). Визуал и ховеры 1:1 с хедерной `NewbieButton`
// (V4DYA настраивал их отдельно) — здесь та же кнопка, но ссылкой и со сменным текстом.
// Переиспользуема: футер + (в перспективе) хедер.
import { SUPPORT_LINK } from "@/data/footerConfig";
...
export function SupportButton({
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`group relative flex h-14 w-40 items-center justify-center focus:outline-none ${className}`}
    >
      ...
      <div className="absolute top-1/2 flex h-8 w-40 -translate-y-1/2 items-center justify-center rounded bg-linear-to-r from-lines-hover via-text-secondary to-lines-hover p-px transition-colors duration-300 group-hover:via-(--primary)">

        {/* 3. Внутренний фон кнопки (Строго #0D0D0F) */}
        <div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-xs bg-(--color-base)">
          ...
          {/* 4. Левый блок: Анимация солдата (26x28px) */}
          ...
          {/* 5. Правый блок: Силуэт города (76x28px) */}
