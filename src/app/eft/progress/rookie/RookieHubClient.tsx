...
// Пока каркас: механики наращиваем следующими срезами. stage.ready — доступен ли этап.
...
const ROOKIE_PATH: RookieStage[] = [
  { id: 'raid', title: 'Что такое рейд', blurb: 'Заход, лут, бой и главное — эвакуация.', ready: false },
...
export function RookieHubClient() {
  // skipHydration: true — регидрируем на клиенте вручную.
  useEffect(() => {
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-baseline justify-between">
          <h2 className="text-sm font-blender-medium uppercase tracking-widest text-text-primary">
            Кто ты в Таркове
          </h2>
          {manualOverride && (
            <button
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {ROOKIE_PATH.map((stage, i) => (
            <div
              key={stage.id}
              className="flex items-center gap-3 rounded-xs border border-lines-hover bg-(--color-base) p-3"
            >
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xs bg-lines-hover font-blender-medium text-xs text-text-secondary">
              ...
              <span className="shrink-0 font-blender-medium text-type-label uppercase tracking-wide text-text-secondary opacity-60">
                {stage.ready ? 'Старт' : 'Скоро'}
              </span>
            </div>
          ))}
        </div>
