// Миграция публичного профиля: добавляет profiles.public_profile (opt-in флаг для
// страницы /u/[username]). Additive + идемпотентно (add column if not exists).
// Запуск: Actions → «migrate-public-profile». Защита CRON_SECRET.
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[cron/migrate-public-profile]", e);
